import { AdminGetUserCommandInput, AdminGetUserCommand, CognitoIdentityProviderClient, InitiateAuthCommand, SignUpCommand, } from '@aws-sdk/client-cognito-identity-provider';
...
export class CognitoService extends IAuthService {
  ...
  public async profile(userId: string): Promise<ProfileInfoResponseDto> {
    const command: AdminGetUserCommandInput = {
      UserPoolId: process.env.COGNITO_USER_POOL_ID,
      Username: userId
    }

    const profile = await this.client.send(new AdminGetUserCommand(command));
    return {
      email: profile.UserAttributes.find(attr => attr.Name === 'email')?.Value,
      name: profile.UserAttributes.find(attr => attr.Name === 'name')?.Value,
      userId: profile.Username
